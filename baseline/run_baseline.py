...
def main(args):
    ...
    def evaluate():
        ...
        for step, input in enumerate(data_eval):
            ...
            adm_jaccard, adm_prauc, adm_avg_p, adm_avg_r, adm_avg_f1 = multi_label_metric(np.array(y_gt), np.array(y_pred), np.array(y_pred_prob))
        ...
        llprint('\tDDI Rate: %.4f, Jaccard: %.4f,  PRAUC: %.4f, AVG_PRC: %.4f, AVG_RECALL: %.4f, AVG_F1: %.4f\n' % (
            ddi_rate, np.mean(jaccard), np.mean(prauc), np.mean(avg_p), np.mean(avg_r), np.mean(avg_f1)
        ))
    ...
    def train():
        ...
        for step, inputs in enumerate(data_train):
            ...
            llprint('\rTrain--Epoch: %d, Step: %d/%d, L_p count: %d, L_neg count: %d' % (epoch, step, len(data_train), prediction_loss_count, neg_loss_count))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        torch.save(model.state_dict(), open(os.path.join(save_path, 'Epoch_%d_JACCARD_%.4f_DDI_%.4f.model' % (epoch, jaccard, ddi_rate)), 'wb'))
        print('')
